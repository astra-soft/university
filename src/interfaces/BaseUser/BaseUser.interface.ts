...
export interface IBaseUserSchema {
	...
	security: {
		...
		/** @property {Array<ILoginAttempt>} loginHistory - Last 10 login attempts. */
...
export interface IBaseUserMethods {
	 ...
	 */
	checkPassword(this: IBaseUser, password: string): void;
 ...
 */
export interface IBaseHashedUser {
...
export abstract class BaseUser extends Document implements IBaseUser {
	 ...
	 */
	checkPassword(this: IBaseUser, password: string): void {
		if (!Auth.verifyHash(password, this.credentials.password)) {
			throw new HttpUnauthorizedError('Invalid password'); // If the password is incorrect, throw a unauthorized error
		}
	}
